...
class DriverServiceRestTest {
    ...
    private Driver create(Integer index) {
        LOG.info("Method create() started {}",
                getClass().getName());
        Driver driver = new Driver();
        driver.setDriverId(index);
        driver.setDriverName("d" + index);
        driver.setDriverDateStartWork(new Date(index).toInstant());
        driver.setDriverSalary(BigDecimal.valueOf(100 + index));
        return driver;
    }
}